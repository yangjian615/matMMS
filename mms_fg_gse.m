...
%
function [t, b_gse, b_dsl, b_smpa, b_bcs, b_omb] = mms_fg_gse(sc, instr, mode, tstart, tend, varargin)
...
%------------------------------------%
	...
	[tf_att, iAtt] = ismember('AttDir', varargin);
	if tf_att
		att_dir               = varargin{iAtt + 1};
		varargin(iAtt:iAtt+1) = [];
	else
		att_dir = pwd();
	end
...
%------------------------------------%
	...
	%
	smpa2dsl = mms_fdoa_xdespin( attitude, t, 'L', [upper(instr) '_123'] );
