...
%   2015-10-21      Removed the FGM parameter (SCM is not affected by DAC 32/64). - MRA
%   2017-01-30      Add 'sc', 'instr', 'mode', and 'optdesc' fields to the output. - MRA
%
function scm = mms_fsm_scm_read(files, trange, fc)
...
%------------------------------------%
	...
	[b_123, t] = mms_fsm_scm_read_l1b(files, trange);
	
	% Categorize the data
	scm = mms_fsm_scm_read_categorize(t, b_123);
...
%------------------------------------%
	...
	% with the data.
	if iscell(files)
		[sc, instr, mode, ~, tstart, ~, optdesc] = mms_dissect_filename( files{1} );
	else
		[sc, instr, mode, ~, tstart, ~, optdesc] = mms_dissect_filename( files );
	end
