%
% Name
%   mms_fsm_bkgd_fit_bigauss_noise
%
% Purpose
...
%   20%, alter change one of the distributions to have 15% amplitude and 250% standard
...
%
function [h_floor, h_sig, fit0] = mms_fsm_bkgd_fit_bigauss_noise(h, f, bins, f0, fc)
...
%------------------------------------%
	...
	%------------------------------------%
		...
		% Adjust initial fit
		%   - One with large peak, small standard deviation
		%   - One with small peak, large standard deviation
		if sqrt( 10^( min( p([2,5]) ) ) ) / sqrt( 10^( max( p([2,5]) ) ) ) < 2
			initFit  = fit( bins', initDist, 'gauss2', ...
			                  'StartPoint', [ p(1) p(2) p(3) 0.15*p(1) p(2) 2.5*p(3)] );
			p        = [ initFit.a1, initFit.b1, initFit.c1, initFit.a2, initFit.b2, initFit.c2 ];
		end
