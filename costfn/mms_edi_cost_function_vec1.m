...
function costFn = mms_edi_cost_function(fv_bpp, pos_bpp, beam_width, grid)
	...
	grid.x       = repmat( reshape( grid.x, 1, dims(1), dims(2) ), N, 1, 1);
	grid.y       = repmat( reshape( grid.y, 1, dims(1), dims(2) ), N, 1, 1 );
	
	% Angle from gun position to each grid point
	%   - Replicate each gun position for each grid point
	grid_angle = atan2( grid.y - repmat( pos_bpp(2,:)', 1, dims(1), dims(2) ), ...
	                    grid.x - repmat( pos_bpp(1,:)', 1, dims(1), dims(2) ) );
	
	% Difference between firing angle and angle to grid point
	%   - Replicate each firing vector at each grid point
	dAngle = grid_angle - repmat( firing_angle', 1, dims(1), dims(2) );
	...
	%   - C  = sum( Ci )
	costFn = sum( ( dAngle ./ repmat( beam_width', 1, dims(1), dims(2) ) ).^2, 1 );
end